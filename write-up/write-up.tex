...
The prepare phase conceptually prepares all sequences from a staring sequence to infinity.
...
Instead we prepare from a some specific sequence number to the local max (max known sequence that has been started) and then reply to the leader's proposer.
