...
In the case of a stable leader Multi-Paxos will have a performance benefit by avoid round trip times when starting Paxos instances in the log.
...
Therefore, whenever he tries propose any new values, he will always be informed by some majority of a decided value by a previous epoch number, if one exists.
\\
...
If the dead server was the leader, then the current server will see know that a new leader will have to be chosen from the alive ones.
Thus, from the pings he just did he knows who the alive servers are and then from them checks the highest server id.
Whoever has the highest server id should act as the leader and that is how servers will know who the new leader is.
If a client needs to know who the leader is, he can be informed by any server of who the server believe who the leader is and the client can eventually get routed to the leader.
Notice that it is possible that in a bad network situation or if there is not a stable leader, that the servers will "fight" to be the leader.
In that case, then we just have servers preparing and increasing their epoch number and dueling for Paxos instances.
This scenario was possible in possible normal Paxos, so in a bad case, the system will degrade and behave like normal Paxos.
